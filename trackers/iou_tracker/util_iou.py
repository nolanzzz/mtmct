...
def load_mot(detections):
    ...
    if type(detections) is str:
        raw = np.genfromtxt(detections, delimiter=',', dtype=np.float32, skip_header=True)
    else:
    ...
    for i in range(start_frame, end_frame+1):
        ...
        for bb, s in zip(bbox, scores):
            dets.append({'bbox': (bb[0], bb[1], bb[2], bb[3]), 'score': s, 'frame_no_cam' : i })
        data.append(dets)
