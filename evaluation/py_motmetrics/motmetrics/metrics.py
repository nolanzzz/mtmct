    ...
    def _compute(self, df_map, name, cache, parent=None):
        ...
        for depname in minfo['deps']:
            ...
            if v is None:
                ...
                start = time.time()
                print("Starting to calculate: {}".format(depname))
                v = cache[depname] = self._compute(df_map, depname, cache, parent=name)
                end = time.time()
                print("Needed time for: {} : {:.3f} s".format(depname,end - start))
            vals.append(v)
...
def calc_hcs(df,hids):

    print("len of df.raw {}".format(len(df.raw)))
    return [ len(df.raw[(df.raw.HId == h)].groupby(level=0)) for h in tqdm(hids) ]

def calc_ocs(df,oids):
    return [ len(df.raw[(df.raw.OId==o)].groupby(level=0)) for o in tqdm(oids) ]
...
def idfp_track_matching(df,id_global_assignment):
    ''''''
    #Assuming that rows are oids and columns are hids
    #Track length may be something with...frequencies
    rids, cids = id_global_assignment['rids'], id_global_assignment['cids']

    idfp_tracks = {}
    for rid,cid in zip(rids,cids):
        idfp_tracks[(rid,cid)] = id_global_assignment['fpmatrix'][rid, cid]

    return idfp_tracks

def idfn_track_matching(df,id_global_assignment):
    ''''''
    #Assuming that rows are oids and columns are hids
    rids, cids = id_global_assignment['rids'], id_global_assignment['cids']
    ...
    idfn_tracks = {}
    for rid,cid in zip(rids,cids):
        idfn_tracks[(rid,cid)] = id_global_assignment['fnmatrix'][rid, cid]

    return idfn_tracks

def idtp_track_matching(df,id_global_assignment,idfn_track_matching, obj_frequencies):
    ''''''
    #Assuming that rows are oids and columns are hids
    #idtp + idfn = overall positives which is equal to obj_frequency I guess

    idx_oids = id_global_assignment["idx_oids"] #To convert row index to oid
    obj_frequencies = obj_frequencies.to_dict()

    idtp_tracks = {}
    for rid_cid, idfn_track in idfn_track_matching.items():
        if rid_cid[0] in idx_oids:
            oid = idx_oids[rid_cid[0]]
            idtp_tracks[rid_cid] = obj_frequencies[oid] - idfn_track
...
def create():
    ...
    m.register(idfp_track_matching)
    m.register(idfn_track_matching)
    m.register(idtp_track_matching)
